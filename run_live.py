...
"""
实盘入口脚本
用法: python run_live.py [--symbols 516630.SH,600000.SH]
"""
...
def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description="实盘入口脚本")
    parser.add_argument(
        "--symbols",
        type=str,
        default="516630.SH",
...
def main():
    ...
    logger.info(f"实盘运行参数: symbols={symbols}")
    ...
    # 3. 初始化 TradeExecutor 并连接 miniQMT
    executor = TradeExecutor(
        mini_qmt_path=config.MINIQMT_PATH,
        account_id=config.ACCOUNT_ID,
        session_id=config.SESSION_ID,
    )
    logger.info("正在连接 miniQMT...")
    if not executor.connect():
        logger.error("连接 miniQMT 失败，退出")
        print("[错误] 连接 miniQMT 失败")
        return
    ...
    try:
        ...
        # 5. 执行一次信号检查和下单
        logger.info("执行信号检查与下单...")
        engine.run_once(symbols)

        # 6. 打印交易日志
        trade_log = engine.get_trade_log()
        if trade_log:
            logger.info(f"本次交易记录共 {len(trade_log)} 条")
            print("\n" + "=" * 60)
            print("交易日志")
            print("=" * 60)
